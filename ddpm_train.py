...
def train(args):
    ...
    logging.basicConfig(filename=os.path.join(train_config['task_name'], "log.txt"), filemode='w', format='%(message)s',
                        level=logging.INFO)
    ...
    for epoch_idx in range(train_config['num_epochs']):
        ...
        for (imgs, labels) in tqdm.tqdm(ddpmloader):
            ...
            labels = labels.to(device)  # 将标签移到与模型相同的设备上
            ...
            # 将类别标签传递给模型
            noise_pred = model(noisy_imgs, t, labels)
